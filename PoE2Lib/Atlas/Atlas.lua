...
--- The range of the Atlas towers.
Atlas.TOWER_RANGE = 12
...
---@return boolean
function Atlas.NodeHasContent(node, contentKey)
    for _, content in pairs(node:getEndgameMapContent()) do
        if content:getKey() == contentKey then
            return true
        end
    end
    return false
