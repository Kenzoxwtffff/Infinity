...
local isInNeverDissipatingDelirium = false
function Delirium.IsInNeverDissipatingDelirium()
    return isInNeverDissipatingDelirium
end

Infinity.Scripting.GetCurrentScript():RegisterCallback("Infinity.OnCachedWorld", function()
    local worldAreaStats = Infinity.PoE2.getGameStateController():getInGameState():getInGameData():getCurrentWorldStats()
    for statId, value in pairs(worldAreaStats) do
        local stat = Infinity.PoE2.getFileController():getStatsFile():getById(statId)
        if stat and value == 1 and stat:getKey() == "map_delirium_fog_never_dissipates" then
            isInNeverDissipatingDelirium = true
            return
        end
    end

    isInNeverDissipatingDelirium = false
end)
