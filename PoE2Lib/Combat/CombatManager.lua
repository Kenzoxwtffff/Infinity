...
function CombatManager.TryTravel(destination, path)
    ...
    local function getPath()
        ...
        if path == nil then
            local worldDest = Infinity.PoE2.WorldTransform.LocationToWorld(destination)
            path = Infinity.Navigation.getPath(Vector.flip(worldDest))
        end
        ...
        local finalIndex = #waypoints
        for i, waypoint in ipairs(waypoints) do
            if waypoint.AreaFlag == NAV_AREA_OFFMESH_CONNECTION then
                finalIndex = i
                break
            end
        end
        for i = finalIndex, 1, -1 do
            ...
            locations[i] = Infinity.PoE2.WorldTransform.WorldToLocation(Vector.flip(pos))
