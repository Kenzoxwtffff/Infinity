...
---@class PoE2Lib.Combat.SkillHandlers.ArtilleryBalistaSkillHandler : PoE2Lib.Combat.SkillHandler
---@overload fun(skillId: number): PoE2Lib.Combat.SkillHandlers.ArtilleryBalistaSkillHandler
local ArtilleryBalistaSkillHandler = SkillHandler:extend()

ArtilleryBalistaSkillHandler.shortName = "Artillery Ballista"

ArtilleryBalistaSkillHandler.description = [[
    This is a skill handler for Artillery Ballista.
]]

ArtilleryBalistaSkillHandler:setCanHandle(function(skill, stats, name, _, _, _, asid)
    return asid == "artillery_ballista_totem"
end)

ArtilleryBalistaSkillHandler.settings = { --
    range = 80,
    radius = 80,
    radiusAuto = true,
    count = 1,
...
ArtilleryBalistaSkillHandler.lastUsing = 0

function ArtilleryBalistaSkillHandler:onPulse()
...
function ArtilleryBalistaSkillHandler:canUse(target)
    if target == nil then
    ...
    if self:countArtilleryBalistaTotems(target) >= self.settings.count then
...
---@param target WorldActor
---@return integer
function ArtilleryBalistaSkillHandler:countArtilleryBalistaTotems(target)
    local tLoc = target:getLocation()
    local count = 0
    for _, actor in pairs(Infinity.PoE2.getLocalPlayer():getDeployedObjects()) do
        if actor:getAnimatedMetaPath() == "Metadata/Monsters/Totems/Ballistas/UpwardBallista.ao" then
...
---@param key string
function ArtilleryBalistaSkillHandler:drawSettings(key)
    local function label(title, id)
        return ("%s##artillery_ballista_skill_handler_%s_%s"):format(title, id, key)
    end
...
return ArtilleryBalistaSkillHandler
