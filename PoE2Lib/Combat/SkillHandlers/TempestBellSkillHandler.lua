...
function TempestBellSkillHandler:canUse(target)
    ...
    if self:getCombo() < self:getComboRequired() then
        return false, "not enough combo"
    end
...
function TempestBellSkillHandler:getCombo()
    for _, buff in pairs(Infinity.PoE2.getLocalPlayer():getBuffs()) do
        if buff:getKey() == "tempest_bell_combo_tracker" and buff:getAssociatedSkillId() == self.skillId then
...
function TempestBellSkillHandler:getComboRequired()
    return self:getStat(SkillStats.RequiredComboStack)
end
