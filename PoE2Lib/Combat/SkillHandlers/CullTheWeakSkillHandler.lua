...
---@class PoE2Lib.Combat.SkillHandlers.CullTheWeakSkillHandler : PoE2Lib.Combat.SkillHandler
---@overload fun(skillId: number): PoE2Lib.Combat.SkillHandlers.CullTheWeakSkillHandler
local CullTheWeakSkillHandler = SkillHandler:extend()

CullTheWeakSkillHandler.shortName = "Cull the Weak"

CullTheWeakSkillHandler.description = [[
    This is special skill handler for the Cull the Weak skill.
]]

CullTheWeakSkillHandler:setCanHandle(function(skill, stats, name, grantedEffectsPerLevel, grantedEffect, activeSkill, activeSkillId)
    return activeSkillId == "frenzied_lunge"
end)

CullTheWeakSkillHandler.settings = { --
    range = 65,
    canFly = false,
    castOnTarget = true,
    onlyCull = true,
...
---@type WorldActor?
CullTheWeakSkillHandler.cachedTarget = nil
CullTheWeakSkillHandler.cachedTargetTick = 0

---@param target WorldActor?
---@return boolean ok, string? reason
function CullTheWeakSkillHandler:canUse(target)
...
function CullTheWeakSkillHandler:getCurrentMaxSkillDistance()
...
---@param target WorldActor?
function CullTheWeakSkillHandler:getImprovedTarget(target)
...
---@param target WorldActor
function CullTheWeakSkillHandler:isCullable(target)
...
function CullTheWeakSkillHandler:use(target, location)
    if self.settings.improvedTargeting then
        target = self:getImprovedTarget(target)
    end
...
---@param target WorldActor?
function CullTheWeakSkillHandler:shouldPreventMovement(target)
...
---@param key string
function CullTheWeakSkillHandler:drawSettings(key)
    local function label(title, id)
        return ("%s##cull_the_weak_skill_handler_%s_%s"):format(title, id, key)
    end
...
return CullTheWeakSkillHandler
