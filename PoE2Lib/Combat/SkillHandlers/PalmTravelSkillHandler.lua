...
function PalmTravelSkillHandler:getBestLocationAlongPath(locations, costs)
    ...
    for _, mob in pairs(Infinity.PoE2.getPotentialCombatTargets()) do
        if (now - (self.lastTargetTimes[mob:getActorId()] or 0) > TARGET_IGNORE_TIME) then
            ...
            if distance >= self.settings.minRange and distance <= self.settings.maxRange and Navigator:getRealDistanceToCellFromPlayer_Quick(mob:getLocation()) <= self.settings.maxRange and not self:isObstructed(origin, mobLoc) then
