...
---@class PoE2Lib.Combat.SkillHandlers.ShockwaveTotemSkillHandler : PoE2Lib.Combat.SkillHandler
---@overload fun(skillId: number): PoE2Lib.Combat.SkillHandlers.ShockwaveTotemSkillHandler
local ShockwaveTotemSkillHandler = SkillHandler:extend()

ShockwaveTotemSkillHandler.shortName = "Shockwave Totem"

ShockwaveTotemSkillHandler.description = [[
    This is a skill handler for Shockwave Totem.
]]

ShockwaveTotemSkillHandler:setCanHandle(function(skill, stats, name, _, _, _, asid)
    return asid == "totem_shockwave"
end)

ShockwaveTotemSkillHandler.settings = { --
    range = 80,
    radius = 80,
    radiusAuto = true,
    count = 1,
...
ShockwaveTotemSkillHandler.lastUsing = 0

function ShockwaveTotemSkillHandler:onPulse()
...
function ShockwaveTotemSkillHandler:canUse(target)
    if target == nil then
    ...
    if self:countShockwaveTotemTotems(target) >= self.settings.count then
...
---@param target WorldActor
---@return integer
function ShockwaveTotemSkillHandler:countShockwaveTotemTotems(target)
    local tLoc = target:getLocation()
    local count = 0
    for _, actor in pairs(Infinity.PoE2.getLocalPlayer():getDeployedObjects()) do
        if actor:getAnimatedMetaPath() == "Metadata/Monsters/Totems/ShockwaveTotem/ShockwaveTotem.ao" then
...
---@param key string
function ShockwaveTotemSkillHandler:drawSettings(key)
    local function label(title, id)
        return ("%s##shockwave_totem_skill_handler_%s_%s"):format(title, id, key)
    end
...
return ShockwaveTotemSkillHandler
